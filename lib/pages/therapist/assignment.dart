...
class _TherapistHome extends State<TherapistHome> {
  ...
  final List<String> category = ['理解', '命名'];
    ...
    if (isLoading) {
      ...
      return Stepper(
                ...
                child: Row(
                  ...
                  children: [
                    dropdownButton(selectedCategory, (value) async {
                      ...
                      selectedLevel = null;
                      setState(() {});
                    }, List.generate(category.length, (index) => DropdownMenuItem(value: category[index], child: Text(category[index])))),
                    // 子項目
                    dropdownButton(
                        selectedSubCategory,
                        selectedCategory != null
                            ? (newValue) {
                                setState(() {
                                  ...
                                  theSub = subCategory.where((element) => element['title'] == newValue).first.data();
                                  level.clear();
                                  content.clear();
                        ...
                        List.generate(subCategory.length, (index) {
                          return DropdownMenuItem(value: subCategory[index]['title'], child: Text(subCategory[index]['title']));
                        })),
                    // 難度
                    dropdownButton(
                        selectedLevel,
                        selectedCategory != null && selectedSubCategory != null
                            ? (newValue) async {
                                ...
                                await Storage().getDataPathList('圖片', selectedLevel!).then((value) {
                                  content = value;
                                });
                                selectedContent = List.generate(content.length, (index) => true);
                                setState(() {});
                              }
                            : null,
                        List.generate(
                            level.length, (index) => DropdownMenuItem(value: level[index], child: Text("Lv.${index + 1}:${level[index]}")))),
                    // 題數
                    dropdownButton(selectedTrial, (value) {
                      setState(() {
                        selectedTrial = value;
                      });
                    }, List.generate(trial.length, (index) => DropdownMenuItem(value: trial[index], child: Text(trial[index].toString())))),
                    const SizedBox(
                      width: 50,
                    ),
                    // 練習次數
        ...
        onStepContinue: () async {
          if (currentStep == 0 && selectedPatient != null) {
          ...
          } else if (currentStep == 1 && selectedCategory != null && selectedSubCategory != null && selectedLevel != null && selectedTrial != null) {
            setState(() {
              currentStep++;
            });
            ...
            DocumentReference ref = await firestore.collection('homeworks').add(Homework(
                    ...
                    level: selectedLevel!,
                    trial: selectedTrial!,
                    content: content.asMap().entries.where((entry) => selectedContent![entry.key]).map((entry) => entry.value).toList(),
                    toDoTimes: selectedTimes,
                    doneTimes: 0)
                .toMap());
            ref.update({'hwId': ref.id});
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(behavior: SnackBarBehavior.floating, content: Text('發送成功，將刷新頁面')));
            }
...
Widget dropdownButton(dynamic value, void Function(dynamic)? onChanged, List<DropdownMenuItem> items) {
  return Container(
    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
